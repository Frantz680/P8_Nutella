...
def my_product_views(request, product_id):
    try:
        My_product_save.objects.get(username_id=request.user.id, product_id=(product_id))
        messages.warning(request, 'Ce produit est déjà dans vos favoris.')
        return render(request, "accounts/my_account.html")

    except ObjectDoesNotExist:
        My_product_save.objects.create(username_id=request.user.id, product_id=(product_id))
